...
//--------------------------------------------------------------------------------
//--------------------------------------------------------------------------------
void Reset_triangulateio( triangulateio & ppIO )
{
	ppIO.pointlist = nullptr;                /* In / out */
	ppIO.pointattributelist = nullptr;       /* In / out */
	ppIO.pointmarkerlist = nullptr;          /* In / out */
	ppIO.numberofpoints = 0;                 /* In / out */
	ppIO.numberofpointattributes = 0;        /* In / out */

	ppIO.trianglelist = nullptr;             /* In / out */
	ppIO.triangleattributelist = nullptr;    /* In / out */
	ppIO.trianglearealist = nullptr;         /* In only */
	ppIO.neighborlist = nullptr;             /* Out only */
	ppIO.numberoftriangles = 0;              /* In / out */
	ppIO.numberofcorners = 0;                /* In / out */
	ppIO.numberoftriangleattributes = 0;     /* In / out */

	ppIO.segmentlist = nullptr;              /* In / out */
	ppIO.segmentmarkerlist = nullptr;        /* In / out */
	ppIO.numberofsegments = 0;               /* In / out */

	ppIO.holelist = nullptr;                 /* In / pointer to array copied out */
	ppIO.numberofholes = 0;                  /* In / copied out */
	
	ppIO.regionlist = nullptr;               /* In / pointer to array copied out */
	ppIO.numberofregions = 0;                /* In / copied out */
	
	ppIO.edgelist = nullptr;                 /* Out only */
	ppIO.edgemarkerlist = nullptr;           /* Not used with Voronoi diagram; out only */
	ppIO.normlist = nullptr;                 /* Used only with Voronoi diagram; out only */
	ppIO.numberofedges = 0;                  /* Out only */
}

//--------------------------------------------------------------------------------
//--------------------------------------------------------------------------------
void Delete_triangulateio( triangulateio & ppIO )
{
	delete ppIO.pointlist;
	ppIO.pointlist = nullptr;                /* In / out */
	delete ppIO.pointattributelist;
	ppIO.pointattributelist = nullptr;       /* In / out */
	delete ppIO.pointmarkerlist;
	ppIO.pointmarkerlist = nullptr;          /* In / out */
	ppIO.numberofpoints = 0;                 /* In / out */
	ppIO.numberofpointattributes = 0;        /* In / out */

	delete ppIO.trianglelist;
	ppIO.trianglelist = nullptr;             /* In / out */
	delete ppIO.triangleattributelist;
	ppIO.triangleattributelist = nullptr;    /* In / out */
	delete ppIO.trianglearealist;
	ppIO.trianglearealist = nullptr;         /* In only */
	delete ppIO.neighborlist;
	ppIO.neighborlist = nullptr;             /* Out only */
	ppIO.numberoftriangles = 0;              /* In / out */
	ppIO.numberofcorners = 0;                /* In / out */
	ppIO.numberoftriangleattributes = 0;     /* In / out */

	delete ppIO.segmentlist;
	ppIO.segmentlist = nullptr;              /* In / out */
	delete ppIO.segmentmarkerlist;
	ppIO.segmentmarkerlist = nullptr;        /* In / out */
	ppIO.numberofsegments = 0;               /* In / out */

	delete ppIO.holelist;
	ppIO.holelist = nullptr;                 /* In / pointer to array copied out */
	ppIO.numberofholes = 0;                  /* In / copied out */
	
	delete ppIO.regionlist;
	ppIO.regionlist = nullptr;               /* In / pointer to array copied out */
	ppIO.numberofregions = 0;                /* In / copied out */
	
	delete ppIO.edgelist;
	ppIO.edgelist = nullptr;                 /* Out only */
	delete ppIO.edgemarkerlist;
	ppIO.edgemarkerlist = nullptr;           /* Not used with Voronoi diagram; out only */
	delete ppIO.normlist;
	ppIO.normlist = nullptr;                 /* Used only with Voronoi diagram; out only */
	ppIO.numberofedges = 0;                  /* Out only */
}
//**** ADD Marek
...
{
    LzLogException("", "Error in Triangle library!")
//  exit(status);
}
